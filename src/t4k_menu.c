...
int T4K_RunMenu(int index, bool return_choice, void (*draw_background)(), int (*handle_event)(SDL_Event*), void (*handle_animations)(), int (*handle_activity)(int, int))
{
    ...
    Uint64 frame_start = 0;       //For keeping frame rate constant
    Uint64 frame_now = 0;
	...
	while (!stop)
	{
		...
		switch (event.type)
		{
			...
			{
			    for (i = 0; i < items; i++) //Handle non-scroll events within button rects
			    {
				if (T4K_inRect(menu->submenu[menu->first_entry + i]->button_rect, event.motion.x, event.motion.y))
				{
		...
		if (old_loc != loc || first_loop || event.key.key == SDLK_F10) {
			...
			T4K_UpdateRect(T4K_GetScreen(), &tmp_rect);
		    }
...
SDL_Surface** render_buttons(MenuNode* menu, bool selected)
{
	...
	menu_items[i] = SDL_CreateSurface(curr_rect.w, curr_rect.h, SDL_PIXELFORMAT_RGBA32);
